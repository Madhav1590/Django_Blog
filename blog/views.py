...
class HomeView(View):
    def get(self,request,*args,**kwargs):
        ...
        popular_obj = Blog.objects.all().filter(status='active',visible=True).annotate(post_count=Count('visit_count')).order_by('-visit_count')
        
        # As per Templates Views
        first_post = featured_obj.first()
        s_post = featured_obj[1]
        last_post = featured_obj[2:]
        p_post = popular_obj[0:6]
        context={
            'post':post_obj,
            'f_post':featured_obj,
            'first':first_post,
